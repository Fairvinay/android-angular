...
public class AuthorizationService {
    ...
    public Intent getAuthorizationRequestIntent(
        ...
        try {
          ...
          authIntent  =  prepareAuthorizationRequestIntent(request, customTabsIntent);
          Logger.info("AuthorizationRequestIntent OKAY  " );
          ...
          Logger.debug(" ActivityNotFoundException ",
            new String[] { "ethier broswer is null", "or selecting wrong browser"});
          Logger.info("ActivityNotFoundException  " );

        }
        return AuthorizationManagementActivity.createResponseHandlingIntent(
                mContext,
                  Uri.parse("https://www.glaubhanta.site/"));
    }
    ...
    private Intent checkNotDisposed() {
        if (mDisposed ) {
          Logger.info("Service has been disposed " );
          Logger.info("Auth Service trying to create new  " );
            //   AuthorizationService(mContext);
          Intent intent
          = new Intent(Intent.ACTION_VIEW);
          Uri requestUri = Uri.parse("https://www.glaubhanta.site/");
          ...
          Intent intent
            = new Intent(Intent.ACTION_VIEW);
          Uri requestUri = Uri.parse("https://reach.glaubhanta.site/");
          intent.setData(requestUri);
          return intent;
        }
    }
    ...
    private Intent prepareAuthorizationRequestIntent(
         ...
         Intent chIntent =         checkNotDisposed();
       if( chIntent.getData().getPath().toString().contains("reach")) {
        if (mBrowser == null) {
          Logger.debug(" browser for auth,is null ",
            new String[]{"ethier broswer is null", "or selecting wrong browser"});
          Logger.info("browser for auth,is null  ");
        ...
        if (mBrowser.useCustomTab) {
          Logger.info("browser for auth,is useCustomTab  ");
          intent = customTabsIntent.intent;
        } else {
          Logger.info("browser for auth,is heuristic or chromw  ");
    ...
    private static class TokenRequestTask
        ...
        protected JSONObject doInBackground(Void... voids) {
            ...
            try {
              ...
              Logger.info("exchange request quest dataa " + queryData);
